...
// --- CORREÇÃO AQUI ---
// 1. Cria uma lista computada que contém APENAS pacientes ativos.
const pacientesAtivos = computed(() => {
  if (!data.value?.data?.entry) {
    return [];
  }
  return data.value.data.entry.filter(p => p.resource.active !== false); // Considera 'undefined' como ativo
});

// 2. A contagem total agora é simplesmente o tamanho dessa lista filtrada.
const totalPacientes = computed(() => {
  return pacientesAtivos.value.length;
});
// --- FIM DA CORREÇÃO ---
