...
func NewHttpTransportLayer(db db.IDB, mux *http.ServeMux, gotenbergUrl string, templateInternePath string) (*http.ServeMux, error) {
	...
	getAvaliation := httptransport.NewServer(
		MakeGetAvaliationEndpoint(avaliationService),
		requests.DecodeGetAvaliationRequest,
		requests.EncodeGetAvaliationRequestResponse,
		httptransport.ServerErrorEncoder(transport_encoding.EncodeError),
	)
	...
	mux.HandleFunc("GET /Avaliation/form", getAvaliationForm.ServeHTTP)
